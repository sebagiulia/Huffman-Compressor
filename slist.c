...
{
  SNodo *inicio = *lista, *nodoNuevo = malloc(sizeof(SNodo)), *temp = inicio;
  nodoNuevo->dato = dato;
  int contador = 1;
  if (posicion == 1) {
    nodoNuevo->sig = *lista;
...
SList slist_desapilar(SList lista)
{
  if (slist_vacia(lista))
    return NULL;
  if (slist_vacia(lista->sig))
    return NULL;
  SList temp = lista;
  for (; temp->sig->sig != NULL; temp = temp->sig);
  SList nodoEliminado = temp->sig;
  temp->sig = NULL;
  free(nodoEliminado);
  return lista;
}