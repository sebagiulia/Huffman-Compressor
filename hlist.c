 ...
 */
void hlist_destruir(HList list, FuncionDestructora destroy)
{
  HNode *nodeToDelete;
  while (list != NULL) {
    nodeToDelete = list;
    ...
    hlist_destruir(nodeToDelete->left, destroy);
    hlist_destruir(nodeToDelete->right, destroy);
    free(nodeToDelete);
  }
}
...
HList hlist_insertar_nodo_ordenado(HList list, HNode *node, FuncionComparadora compare)
{
  ...
  for (; nodeTemp->next != NULL; nodeTemp = nodeTemp->next) {
    if (compare(node->data, nodeTemp->next->data) < 0) { // Notemos que no ponemos <= 0 ya que de esta forma, la inserion para formar arboles seria desbalanceada
      aux = nodeTemp->next;
      nodeTemp->next = node;
      node->next = aux;
      return list;
